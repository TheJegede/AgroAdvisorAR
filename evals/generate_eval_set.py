...
Samples chunks from raw_pdfs/, calls Groq (LLaMA 3.3 70B) to generate realistic
farmer queries, writes evals/eval_set.jsonl.
...
# 100 total, proportional to corpus size (rice ~72, soybeans ~66, poultry ~16 PDFs)
...
def main():
    ...
    print(f"Generating queries for {len(sampled)} chunks via Groq (llama-3.3-70b-versatile)...")
    results = []
    ...
    for i, doc in enumerate(sampled):
        ...
        time.sleep(2)  # ~30 RPM free tier — 2s gives headroom
        if (i + 1) % 10 == 0:
