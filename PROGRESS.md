...
## ▶▶ RESUME HERE (next session) — RAGAS Phase 1 BUILT+RUN; pick next lever
- **What just shipped:** RAGAS diagnostic eval Phase 1 built TDD via `/build` (plan `docs/superpowers/plans/2026-06-12-ragas-diagnostic-eval-phase1.md`). Tasks 0–6 = code ($0, 8 pure tests green), Task 7 = the paid matrix run (Taiwo OK'd). 7 task-commits on `feat/ragas-diagnostic-eval` — **NOT merged to main, NOT pushed.** New files: `evals/ragas_eval.py`, `evals/test_ragas_eval.py`, `evals/test_answer_eval_capture.py`, `evals/requirements-ragas.txt`; modified `evals/answer_eval_full.py` (`_capture_fields` → dump carries `answer`+`contexts`).
- **The matrix (n=38, B1-on arm, Gemini-2.5-flash judge + local gte embedder):** see "RAGAS DIAGNOSTIC MATRIX — RESULTS" section below. Headline: faith 0.72 / ans_rel 0.78 / ctx_precision(ref-free) 0.71 / ctx_recall 0.16 (rice 0.00\* provisional). Confirms the existing story — retrieval precision decent (NOT the bottleneck), generation faithfulness the moderate ceiling, low ctx_recall is largely an eval-artifact. **Diagnostic, not a lever — raised no ceiling, as designed.**
- **NEXT (Taiwo's call — generation levers, B2 closed the format-tax route):** (1) **Curate rice gold labels** in `eval_set_v2_clean.jsonl` (data fix; un-provisions rice ctx_recall + makes the rice headline trustworthy — highest-leverage). (2) **Phase C generation model swap** (ongoing prod cost — present numbers, Taiwo decides). (3) **B3 `source_quote`** rate-grounding lever (rate-grounding 46% < 80% = live headroom). Lever series: L1=NO-OP, L2=WIN, L3=WIN, B1=WIN, B2=DISPROVEN/closed.
- **Merge/push decision pending:** branch is eval-only (no `backend/**` → push does NOT trigger HF deploy). Merge to main per branch-safety rule only after Taiwo OK.
