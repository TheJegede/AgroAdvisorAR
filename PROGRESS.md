...
## ▶▶ RESUME HERE (next session) — Rice Gold Curation Tasks 1–7 BUILT ($0); Task 8 re-measure COST-GATED
- **STATUS 2026-06-13:** `/build` of the plan ran TDD on `feat/rice-gold-curation`. **Tasks 1–7 DONE, all $0, 6 pure tests green, 7 per-task commits.** New module `evals/rice_gold_curation.py` (flag → independent keyword candidate-search over `corpus_v3` → apply → audit → validate). Curated output `evals/eval_set_v2_clean_rice.jsonl` written + validated: **198 → 191 rows (7 dropped, 63 repointed), residual TOC golds = 0, rice 111 → 104.**
- **Task 6 human gate (Taiwo answered):** dropped **7 cross-crop mislabels** (1 soybean auto + 6 confirmed: 2 corn / 2 soybean / 2 wheat questions mis-filed in the rice namespace — these were `EVAL_MISLABEL` per the 2026-06-13 diagnosis); accepted keyword top-1 for the remaining 63 repoints. Decisions table `evals/rice_curation_decisions.json` + audit `docs/superpowers/2026-06-12-rice-gold-curation-audit.md`.
- **▶ NEXT = Task 8 (the ONLY token-spend, HOLD for explicit Taiwo OK):** gen re-run n=40 seed=7 (DeepInfra 70B + Gemini judge, B1-on arm) on `evals/eval_set_v2_clean_rice.jsonl` → new honest rice corr/faith headline; optional RAGAS re-run un-provisions rice `context_recall`. Command in plan Task 8 Step 2. Record results here + gitignore `evals/_capture_rice_curated.jsonl` after.
- **Plan:** `docs/superpowers/plans/2026-06-12-rice-gold-curation-phase1.md`. **Spec:** `docs/superpowers/specs/2026-06-12-rice-gold-curation-design.md`. Branch `feat/rice-gold-curation` (eval-only → push = no deploy). NOT yet merged to main.
