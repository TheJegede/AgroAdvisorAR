...
- **NEXT (per plan):** B2 format-tax probe (free-form gen → cheap formatter, eval-only, ~$0.05) — would also settle the Phase C (model swap) question cheaply. B3 (`source_quote`) likely redundant after B1 (quote-into-analysis is its lite form) — re-check before building. Latency note: analysis generates first → delays first streamed visible field; acceptable at the 45–50s gen ceiling, revisit with token-streaming work. **Backend changed → push to main triggers HF deploy; NOT yet pushed.**
