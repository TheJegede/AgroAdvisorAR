...
## ▶▶ RESUME HERE (next session) — Phase C gen-model swap RUN: free-model engine-lever EXHAUSTED (incumbent 70B Pareto-best); generation levers CLOSED under the free constraint
- **STATUS 2026-06-13 (Phase C eval-only probe BUILT + RUN on branch `feat/phase-c-gen-model-probe-v2`, NOT merged; eval-only → push fires NO deploy):** added `--gen-model <id>` arm to `evals/answer_eval_full.py` (`_apply_gen_model_override`: repoints `config.DEEPINFRA_MODEL` + clears `rag._deepinfra_llm` cache → swaps ONLY generation, retrieval+judge unchanged). 2 unit tests green, $0 build, commit `74d5ff0`. Candidates chosen to live on **both** DeepInfra (cheap eval) AND Groq free tier (→ $0 productionization).
- **▶ PAIRED RESULT (n=38 identical items, `eval_set_v2_clean.jsonl` seed=7, DeepInfra gen + independent Gemini judge, B1-on):**

  | arm | corr | faith | vs 70B (paired) |
  |---|---|---|---|
  | **`llama-3.3-70b` (incumbent free)** | **30.3%** | **71.1%** | baseline |
  | `openai/gpt-oss-120b` | 23.7% | 67.1% | **−6.6 corr / −3.9 faith** (helped 6/hurt 8 corr; 9/hurt 12 faith) |
  | `Qwen/Qwen3-32B` | — | — | **INCOMPATIBLE** — emits `<think>` reasoning tokens → `with_structured_output(AdvisoryDraft)` JSON parse fails → 13/13 ALL SKIPPED, killed |

- **THE FINDING (NIW/arXiv-quotable negative):** among **free-tier-serveable** models, the incumbent `Llama-3.3-70B` is **Pareto-best** for this constrained-schema RAG generation path. A larger free MoE (`gpt-oss-120b`, 120B) **regressed** both corr (−6.6) and faith (−3.9), soybeans worst (38→25); a free reasoning model (`Qwen3-32B`) is **structurally incompatible** (think-token leakage breaks the JSON-locked schema, 100% skip). gpt-oss n=40 standalone = corr 23.8/faith 63.8/supp 0% (never abstains, confidence 1.00 = overconfident-but-worse). **Free-model engine-lever = EXHAUSTED.** Only untried engine lever = a PAID frontier model (Anthropic/Gemini-pro) — Taiwo ruled this OUT (ongoing prod cost; `feedback_avoid_token_cost`). → **generation levers fully CLOSED under the free-tier constraint.** Honest ceiling stands: corr ~30% (single-gold) / ~61% (Phase-2 answer-key) / faith ~71%.
- **▶ THESIS FINALIZED 2026-06-13 (write-up done, both framings drafted — all LOCAL/gitignored, consistent with PRDs):** consolidated all 6 findings into `docs/thesis-evidence-spine.md` (3 pillars: retrieval CLOSED / generation ceiling EXHAUSTED inc. Phase C / single-gold artifact +21.5pts; §6 = the ONLY citeable honest-numbers table, retires 0.6565 MRR + 35% self-judged). Then BOTH venue framings off the shared spine: `docs/niw-evidence-memo.md` (Dhanasar 3-prong — substantial-merit+national-importance / well-positioned / waiver-benefit; impact + sole-authorship emphasis) + `docs/arxiv-draft.md` (methodology-contribution emphasis: single-gold artifact + circularity-guarded multi-reference + independent-judge debiasing + rigorous negatives; abstract + 8 sections). **Confirmed confidence UX already SHIPPED** (4 components: ConfidenceBadge / per-claim NLIConfidenceBadge / ConfidenceExplainer / LowConfidenceBanner) → not a gap.
- **▶ NEXT (Taiwo's call):** generation + retrieval both CLOSED; thesis spine + NIW memo + arXiv draft exist. Remaining: (1) **refine the two drafts** (figures: lever-map table, Pareto scatter corr-vs-faith, single-gold-vs-answerkey paired plot; full citations); (2) **tighten evidence** — validate more answer keys (83/~197) to sharpen ~61% + per-crop; (3) data/UX leftovers (rice gold artifact understood; latency token-streaming; pilot readiness). NO productionization from Phase C. Lever series: L1=NO-OP, L2=WIN, L3=WIN, B1=WIN, B2=DISPROVEN, Phase2=MEASUREMENT-FIX, **Phase C=NO-LIFT (free-model ceiling confirmed)**.
- **Artifacts (gitignored dumps):** `evals/_capture_genmodel_baseline70b.jsonl`, `evals/_capture_genmodel_gptoss.jsonl`, logs `evals/_phaseC_{baseline,gptoss,qwen}.log`. Plan `docs/superpowers/plans/2026-06-13-phase-c-gen-model-probe.md` (written for an Anthropic sweep; ACTUAL build pivoted to free-Groq/DeepInfra per Taiwo's cost veto — archive note on merge).
- **Run note:** Python block-buffers stdout when redirected → a buffered run looked "stuck" for 25min (was progressing). Relaunch with `python -u` + `PYTHONUNBUFFERED=1` for live per-item visibility; set `LANGCHAIN_TRACING_V2=false` to silence LangSmith 429 trace-quota spam.

---

## ▶ (prior) RESUME — Phase 2 answer-key grading MERGED to main; single-gold artifact PROVEN (+21.5 pts) on human-validated ground truth
- **STATUS 2026-06-13 (Phase 2 DONE + MERGED + PUSHED to main `4edbb91` --no-ff, eval/docs only → NO HF/Vercel deploy; branch `feat/phase2-ragas-ground-truth` safe to delete; all 100 keys verdicted across both validation docs, 0 remaining):** `/build` of `docs/superpowers/plans/2026-06-13-phase2-ragas-ground-truth.md` ran inline TDD. **Tasks 1–7 DONE.** Tasks 1–4,6 = $0 (9 ground_truth tests + 4 compute_correctness tests green, per-task commits); package `evals/ground_truth/` = answer-key store + grounded synthesis prompt + independent-Gemini answer-key judge + cost-gated `synth.py` + `--grade-mode {gold,answerkey,both}` in `answer_eval_full.py` + RAGAS `--with-answer-key` (AnswerCorrectness). Default `gold` path byte-unchanged (16 regression tests green).
...
- **▶ NEXT (Taiwo's call):** (1) ~~Merge to main~~ **DONE 2026-06-13 (`4edbb91`)** — archive plan to `plans/completed/` next pass. (2) **Phase C generation model swap** — answerkey corr 61% is the honest headroom signal; gen is still the ceiling. (3) Optionally validate more keys (83/197 scored; remaining 114 unsynthed-or-INSUFFICIENT) + run RAGAS `--with-answer-key` for `answer_correctness`. Lever series: L1=NO-OP, L2=WIN, L3=WIN, B1=WIN, B2=DISPROVEN, **Phase 2 answer-key grading = MEASUREMENT FIX (single-gold artifact = +21.5 pts proven)**.
- **Artifacts:** dump `evals/_capture_answerkey_paired.jsonl` (gitignored). Tracked: keyed subset `evals/eval_set_v2_clean_keyed.jsonl`, answer keys `evals/ground_truth/answer_keys.jsonl`, validation docs `docs/superpowers/findings/2026-06-13-phase2-answer-key-validation{,-round2}.md`.
...
> Plan `docs/superpowers/plans/2026-06-13-rice-diagnosis-b2-format-tax.md` executed. Findings doc: `docs/superpowers/findings/2026-06-13-rice-diagnosis-findings.md`. Evals-only (no prod code) → push does NOT trigger HF deploy.
...
> Ran the planned failure-split (`docs/superpowers/plans/2026-06-12-corpus-gap-retrieval-split.md`). **Conclusion: the gap is GENERATION-SPECIFICITY, not corpus coverage.** The prior "NEXT = corpus-coverage gap" hypothesis is **CONTRADICTED** — do not re-propose corpus/re-ingest as the soybean lever. Full writeup: `docs/superpowers/findings/2026-06-12-corpus-gap-findings.md`.
