...
## ▶▶ RESUME HERE (next session) — BUILD Rice Gold Curation (Phase 1.5)
- **Plan:** `docs/superpowers/plans/2026-06-12-rice-gold-curation-phase1.md` (TDD, 8 tasks, handoff-ready). **Spec:** `docs/superpowers/specs/2026-06-12-rice-gold-curation-design.md`.
- **Branch:** `feat/rice-gold-curation` — spec+plan committed here, **NOT main**. `git switch feat/rice-gold-curation` before starting.
- **How:** new session → `/build` pointed at the plan path above (the plan's "Handoff" section is the airtight cold-start checklist).
- **What:** curate the rice gold labels — **70 of 111 rice rows** (63%) point at non-answer-bearing "br wells … research studies" yearly-volume TOCs (this is why rice corr is stuck at 18% — an EVAL artifact, per the 2026-06-13 diagnosis). Re-point each off the TOC onto the dedicated topical rice doc drawn from `corpus_v3` by an **independent keyword search** (NOT the prod gte embedder, blind to eval dumps = circularity guard) + drop the wrong-crop mislabels. Output = new `evals/eval_set_v2_clean_rice.jsonl` (originals untouched) + a human-review audit table. **DATA fix, not a pipeline lever** — touches no `backend/`/prompt. Un-provisions rice RAGAS `context_recall` + makes the rice headline NIW/arXiv-honest.
- **Cost:** Tasks 1–7 = **$0** (pure helpers + deterministic draft + human review + file writes + asserts). **Task 8 is the only token-spend** (gen re-run n=40 + optional RAGAS matrix) → **HOLD for explicit Taiwo OK** even mid-build. **Task 6 is a human-in-the-loop gate** — surface the draft audit to Taiwo, don't invent agronomy silently.
- **Why this first (over Phase C model-swap / B3):** $0, unblocks a trustworthy rice headline that every other lever's rice delta is currently unreadable against, and is the hand-audit Phase 2 would otherwise need.
