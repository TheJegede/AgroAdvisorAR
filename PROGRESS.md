...
## ▶▶ RESUME HERE (next session) — Rice Gold Curation BUILT + RE-MEASURED; finding = rice corr is a SINGLE-GOLD artifact, faith is the trustworthy rice signal
...
- **▶ NEXT (Taiwo's call):** (1) **Phase 2 RAGAS synthetic ground-truth + human-validated subset** = the real fix for a trustworthy rice corr + un-provisions rice `context_recall` + enables `answer_correctness` (absorbs this curation loose end). (2) **Phase C generation model swap** (ongoing prod cost) — faith 75.7% is the moderate ceiling. (3) Merge `feat/rice-gold-curation` to main (eval-only, no deploy) — curated set + module are a clean $0 asset even though corr didn't move. Lever series: L1=NO-OP, L2=WIN, L3=WIN, B1=WIN, B2=DISPROVEN, **rice-gold-curation = data asset, corr-unmovable by single-gold (faith is the signal)**.
- **Artifacts (gitignored):** dump `evals/_capture_rice_curated.jsonl`, log `evals/_capture_rice_curated.log`. Plan `docs/superpowers/plans/2026-06-12-rice-gold-curation-phase1.md`, spec `…/specs/2026-06-12-rice-gold-curation-design.md`. Branch `feat/rice-gold-curation`, NOT yet merged.
