...
function countyColor(count, maxCount) {
  ...
  const t = Math.sqrt(count / maxCount) // sqrt scale — smaller counts still visible
...
export default function ARCountyMap({ countyData = [] }) {
    ...
    <div className="relative w-full" style={{ minHeight: 220 }}>
              ...
              .map(geo => {
                ...
                return (
                  <Geography
                    key={geo.rsmKey}
                    geography={geo}
                    fill={countyColor(count, maxCount)}
                    stroke="#ffffff"
                    strokeWidth={0.8}
                    onMouseEnter={() => {
      ...
      {tooltip && (
        <div className="absolute top-2 right-2 bg-white border border-gray-200 rounded-md px-3 py-1.5 text-xs shadow-sm pointer-events-none dark:bg-hc-surface dark:border-hc-border dark:text-hc-fg">
          ...
          <span className="text-gray-500 dark:text-hc-fg ml-2">{tooltip.count} queries</span>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 mt-2 text-xs text-gray-500 dark:text-hc-fg px-1">
        ...
        <div
          ...
          style={{ background: 'linear-gradient(to right, #EEF2EF, #2D6A4F)' }}
        />
        <span>{maxCount}</span>
