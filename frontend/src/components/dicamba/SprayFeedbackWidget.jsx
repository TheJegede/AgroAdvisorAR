...
import { useSprayCheck } from '../../hooks/useSprayCheck'

export default function SprayFeedbackWidget({ recordId }) {
  const { t } = useLang()
  const { submitFeedback, feedbackSubmitting } = useSprayCheck()
  ...
  async function handleSend() {
    ...
    const res = await submitFeedback({ recordId, rating: pendingRating, comment })
    if (res.ok) {
      setStatus('done')
      setComment('')
  ...
  return (
    <div className="mt-6 pt-4 border-t border-gray-100 dark:border-t-2 dark:border-hc-border text-sm" data-testid="spray-feedback-widget">
      {status === 'done' ? (
          ...
          {pendingRating !== null && (
            <div className="mt-3 flex flex-col gap-2">
              ...
              <div className="flex items-center gap-3">
                ...
                {status === 'error' && (
                  <span className="text-xs text-arred dark:text-hc-danger font-bold" data-testid="feedback-error-msg">{errorMsg}</span>
                )}
              </div>
            </div>
          )}
