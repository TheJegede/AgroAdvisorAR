...
export default function ChatInput({ onSubmit, disabled }) {
  ...
  return (
    ...
    >
      {showCounter && (
        <p className={`text-xs mb-2 text-right ${remaining < 0 ? 'text-arred' : 'text-gray-400'}`}>
          {remaining} {t.charsRemaining}
        </p>
      )}

      <div className="flex items-end gap-2 rounded-2xl border border-gray-200 px-3 py-2 bg-white
        focus-within:border-field focus-within:ring-1 focus-within:ring-field/20 transition-all">

        {/* Paperclip — decorative */}
        <span className="flex-shrink-0 text-gray-400 p-1 mb-0.5" aria-hidden>
        ...
        <textarea
          ...
          className="flex-1 resize-none bg-transparent text-base text-charcoal
