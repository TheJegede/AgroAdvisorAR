...
async def _cached_fetch(cache_key: str, fetch_fn, ttl: int | None = None):
    """Check cache, call fetch_fn on miss, cache only on truthy result."""
...
async def fetch_ssurgo(fips: str) -> dict:
    ...
    return await _cached_fetch(f"ssurgo:{fips}", _fetch) or _unavailable()
...
async def fetch_noaa(fips: str) -> dict:
    ...
    return await _cached_fetch(f"noaa:{fips}", _fetch) or _unavailable()
...
async def fetch_usgs_well(fips: str) -> dict | None:
    ...
    async def _fetch():
        ...
        iv_value = iv_data.get("value") or {}
        series = iv_value.get("timeSeries") or []
        if not series:
            return None
        ...
        def _dist(ts):
            ...
            geo_location = source_info.get("geoLocation") or {}
            geo = geo_location.get("geogLocation") or {}
        ...
        current_depth_m = round(current_depth_ft * FEET_TO_METERS, 3)
        ...
        try:
            ...
            p75: float | None = None
            p90: float | None = None
            stat_value = stat_data.get("value") or {}
            for s in (stat_value.get("timeSeries") or []):
                name = s.get("name", "")
                values_container = s.get("values") or [{}]
                vals = values_container[0].get("value") or []
    ...
    return await _cached_fetch(f"usgs_well:{fips}", _fetch, ttl=86400)
